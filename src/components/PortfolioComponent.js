...
import proyectos from "../data/proyectosEsp.json";
import projectsEng from "../data/proyectosEng.json";
...
export const PortfolioComponent = () => {
    ...
    return (
        <Container fluid className={darkMode ? 'darkBody' : 'lightBody'}>
                                ...
                                if (elementosEnFila < 3 && index2 >= index) {
                                    ...
                                    return <Col lg={3}>
                                        <PortfolioItemComponent
                                            ...
                                            mejoras={proyecto.mejoras}
                                            tecnologiasUtilizadas={proyecto.tecnologiasUtilizadas}
                                            pageLink={proyecto.pageLink}
            ...
            })) : (projectsEng.map((proyecto, index) => {
                if (index % 3 === 0) {
                    let elementosEnFila = 0;
                    return <Row className='filaPortfolio'>
                        {
                            projectsEng.map((proyecto, index2) => {
                                if (elementosEnFila < 3 && index2 >= index) {
                                    elementosEnFila++;
                                    return <Col lg={3}>
                                        <PortfolioItemComponent
